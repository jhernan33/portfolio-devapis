...
The **downloadable CVs** in `src/documentos/` are generated by `tools/generar-cv-ats.py`
(edit the script, never the .docx/.pdf). Single column, Arial, literal-hyphen bullets, no
tables or images: an ATS reader must be able to parse them. Experience is split into the
main role and a **concurrent engagements** block, because four overlapping jobs read as a
date error. Figures live in the `METRICAS` table, empty until the holder confirms each one
with evidence; a bullet with an unconfirmed metric falls back to its qualitative wording.
`--marcadores` writes a draft with `[CONFIRMAR: ___]` to `.borradores/`, **never** to
`src/documentos/`, which Nginx publishes — a CI guard fails if a published CV carries a
marker.
